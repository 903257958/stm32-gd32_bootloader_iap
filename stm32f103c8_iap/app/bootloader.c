...
static void __handle_eflash_program_download_select(uint8_t *data, uint16_t len)
{
	if (len == 1)
	{
		...
		if (data[0] >= 0x31 && data[0] <= 0x39) 
		{
			...
			w25q128.block_erase_64kb(&w25q128, flash_a_update_cb.eflash_block_index);	// 先擦除要写入程序的块
...
static void __bootloader_clear(void)
{
	...
	// gpio_deinit(w25q128.config.sck_port);
	// gpio_deinit(w25q128.config.miso_port);
	// gpio_deinit(w25q128.config.mosi_port);
	// gpio_deinit(w25q128.config.cs_port);
}
