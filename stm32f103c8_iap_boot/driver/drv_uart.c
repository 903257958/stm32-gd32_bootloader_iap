...
static char *uart_recv_str(uart_dev_t *dev);
static int uart_recv_data(uart_dev_t *dev, uint8_t **data, uint32_t *len);
...
static char *uart##x##_recv_str_impl(void)							\
{															   	    \
	const uart_hw_info_t *hw_info = uart_get_hw_info(uart_periph);  \
	uart_dev_t *dev = g_uart_priv[hw_info->idx].dev;				\
	return uart_recv_str(dev);								   		\
}																	\
static int uart##x##_recv_data_impl(uint8_t **data, uint32_t *len)	\
{															   	    \
	const uart_hw_info_t *hw_info = uart_get_hw_info(uart_periph);  \
	uart_dev_t *dev = g_uart_priv[hw_info->idx].dev;				\
	return uart_recv_data(dev, data, len);							\
}																	\
static const uart_ops_t uart##x##_ops = { 							\
    .printf = uart##x##_printf_impl, 								\
    .send = uart##x##_send_impl, 									\
    .recv_str = uart##x##_recv_str_impl, 							\
    .recv_data = uart##x##_recv_data_impl, 							\
