...
/**
 * @brief   串口接收字符串
 * @details 从环形缓冲区中取出一段连续数据，将其复制到调用方提供的缓冲区 str 中，并在末尾自动补 '\0'。
 * 			本函数不对 str 的大小进行任何检查，复制长度完全取决于当前数据段的实际长度。
 * 			本实现仅适用于调用方能够明确控制内存及数据量的场景，风险完全由调用方自行承担。
 *          调用方必须确保：
 *          1. str 具备足够的空间容纳本次复制的全部数据以及结尾的 '\0'；
 *          2. 了解本函数为“不安全接口”，可能造成缓冲区溢出；
 *          3. 若需要安全的长度控制，应改用上层已提供的安全版本 recv_data()。
 *
 * @param[in]  dev uart_dev_t 结构体指针
 * @param[out] str 调用方提供的接收缓冲区，需具备足够容量
 *
